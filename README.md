...
5. **The silence threshold must be calibrated against the actual tablet mic and room — not guessed or copied from a different device.** A fixed volume number tuned on one microphone/environment can be completely wrong on another: if it's set too high relative to actual speaking volume on this hardware, the visitor's speech never gets recognized as "speech" at all, and recording will silently run to the hard max every time — even though the audio itself is captured correctly. The visible symptom is: no meaningful volume-based feedback/logging during speech, followed by the max-length cutoff firing on every recording, even for short utterances.

   To calibrate correctly:
   - Record a few seconds of the room with no one talking, and note the mic's volume/amplitude reading during that silence.
   - Record a few sample utterances at a normal speaking volume/distance from the mic, and note the volume reading during actual speech.
   - Set the threshold roughly halfway between the two, with a margin — not right at the edge of either range. If speech and silence readings are too close together to find a clean gap, the mic gain/placement may need adjusting, not just the threshold number.
   - Test this on the real target tablet, in the real deployment room, not a laptop or a different room — mic sensitivity and ambient noise both vary by device and location.
